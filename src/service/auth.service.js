...
class AuthService {
  // 사용자 로그인
    login(user) {
        return axios
            .post(process.env.VUE_APP_BASE_URL + 'auth/signin', user)  // 로그인 엔드포인트에 POST 요청을 보냄
            .then(res => {
                if(res.data) {
  ...
  // 사용자 로그아웃
    logout() {
        ...
        return axios
            .post(process.env.VUE_APP_BASE_URL + 'auth/signout', user) // 로그아웃 엔드포인트에 POST 요청을 보냄
            .then((res) => {
  ...
  // 사용자 등록
    register(user) {
        return axios
        .post(process.env.VUE_APP_BASE_URL + 'auth/signup', user); // 등록 엔드포인트에 POST 요청을 보냄
    }
